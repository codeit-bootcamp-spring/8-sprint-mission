...
public interface BinaryContentApi {
  ...
  @Operation(summary = "첨부 파일 조회 (바이트 포함, 프론트 첨부 미리보기용)")
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = "200", description = "첨부 파일 조회 성공 (bytes: base64)",
          content = @Content(schema = @Schema(implementation = BinaryContentWithBytesDto.class))
      ),
      @ApiResponse(
  ...
  ResponseEntity<BinaryContentWithBytesDto> find(
