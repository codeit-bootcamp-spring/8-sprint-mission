...
function m1({channel: r}) {
  ...
  const x = async (P, I) => {
        try {
          // 1. 서버에 /download API 호출 (응답은 R에 담김)
          const R = await he.get(`${Xe.apiBaseUrl}/binaryContents/${P}/download`, {
            responseType: "blob" // 반드시 blob으로 설정해야 깨지지 않습니다.
          });

          // 2. 응답받은 데이터(R.data)를 사용하여 Blob 객체 L을 생성 (선언문 확인!)
          const L = new Blob([R.data], {type: R.headers["content-type"]});

          // 3. 다운로드를 위한 임시 URL 생성
          const V = window.URL.createObjectURL(L);
          const F = document.createElement("a");
          F.href = V;
          F.download = I; // 전달받은 파일명(I)으로 설정
