...
public class AuthController {
  ...
  private void expireSessionsByUsername(String username) {
    sessionRegistry.getAllPrincipals().stream()
        .filter(principal -> principal instanceof UserDetails ud
                             && ud.getUsername().equals(username))
        .flatMap(principal -> sessionRegistry.getAllSessions(principal, false).stream())
        .forEach(sessionInfo -> {
          sessionInfo.expireNow();
          log.info("{} 세션 만료 처리: username={}, sessionId={}",
              AuthConstants.LOG_PREFIX_CSRF_TOKEN, username, sessionInfo.getSessionId());
        });
  }
}
