...
/**
 * 요청의 Authorization 헤더에서 Bearer Access Token을 읽어 검증하고,
 * 유효할 경우 SecurityContext에 인증 객체를 저장하는 JWT 기반 커스텀 인증 필터.
 * OncePerRequestFilter를 상속받아 디스패처 서블릿의 여러 처리 과정(포워딩, 인클루드 등)에서
 * 중복 실행되는 것을 방지하고, 하나의 HTTP 요청당 정확히 한 번만 인증 검증이 수행되도록 보장한 것이 핵심.
 * 이 커스텀 필터는 SecurityConfig에서 우리가 알던 UsernamePasswordAuthenticationFilter 이전에 위치할 것이며,
 * 인증 실패 시 401 JSON 응답을 반홚하여 클라이언트에게 명확한 오류 상태를 전달한다.
 */
@Slf4j
@Component
        ...
        try {
            ...
            if (StringUtils.hasText(token)) {
                ...
                if (tokenProvider.validateAccessToken(token)) {
                    ...
                    // 폐기되지 않았다면 유효한 토큰이므로 토큰 주체(subject)로 저장된 사용자 명을 추출한다.
            ...
            log.info("[JwtAuthentication] 예외 발생: {}", e.getMessage());
            SecurityContextHolder.clearContext();
            sendUnauthorized(response, "JWT authentication failed");
            return;
        }

        // JWT 기반 인증 후 다음 필터로 체인을 이어감
