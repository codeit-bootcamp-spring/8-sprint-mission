...
/**
 * JWT 토큰의 상태(활성/폐기/회전)를 데이터베이스에 저장/조회하는 레지스트리(저장소)
...
public class JwtSessionRegistry {
    ...
    public JwtSessionRegistry(JwtTokenRepository jwtTokenRepository) {
        log.info("[JwtSessionRegistry] 생성자 호출됨: JwtTokenRepository 주입");
        this.jwtTokenRepository = jwtTokenRepository;
    }

    /**
     * 신규 토큰 메타데이터 저장.
     * 록그인/재발급 시 발급된 AT/RT를 DB에 기록한다.
     */
    public void register(JwtTokenEntity token) {
        log.info("[JwtSessionRegistry] register 호출됨: jtl={}, type={}", token.getJti(), token.getTokenType());
        // 토큰 엔티티를 영속화한다.
        jwtTokenRepository.save(token);
        log.info("[JwtSessionRegistry] register 완료: 저장됨");
    }
    ...
    public void revokeAllByUsername(String username) {
        log.info("[JwtSessionRegistry] revokeAllByUsername 호출됨: username={}", username);
        // 사용자명으로 모든 토큰을 조회한다.
        ...
        log.info("[JwtSessionRegistry] revokeAllByUsername 완료: count={}", tokens.size());
    }
    ...
    public boolean isRevoked(String jti) {
        boolean result = jwtTokenRepository.findById(jti)
                ...
                .orElse(false);
        log.info("[JwtSessionRegistry] isRevoked 호출됨: jti={}, result={}", jti, result);
        return result;
    }
    ...
    public void markReplaced(String oldJti, String newJti) {
        log.info("[JwtSessionRegistry] markReplaced 호출됨: oldJti={}, newJti={}", oldJti, newJti);
        jwtTokenRepository.findById(oldJti).ifPresent(t -> {
            t.setRevoked(true);
            t.setReplacedBy(newJti);
            jwtTokenRepository.save(t);
            log.info("[JwtSessionRegistry] markReplaced 완료: oldJti 폐기 및 교체 표시");
        });
    }
    ...
    public void revokedByJti(String jti) {
        ...
        jwtTokenRepository.findById(jti).ifPresent(t -> {
            t.setRevoked(true);
            jwtTokenRepository.save(t);
            log.info("[JwtSessionRegistry] revokeByJti 완료: jti={}", jti);
        });
    }
}
